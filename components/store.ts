...
export const useStore = create<Store>((set, get) => ({
  ...
  setUser: (_user: UserStore) =>
    set((state) => ({ user: { auth: _user.auth, data: _user.data } })),
  setUserLoading: (userLoading: boolean) =>
    set((state) => ({ userLoading: userLoading })),
  cache: {
    blogs: {},
    users: {},
    posts: {},
  ...
  title: 'reauthor',
  addPostToCache: (post: Post) =>
    set((state) => {
      const newCache = state.cache
      newCache.posts[post.slug] = post
      return { cache: newCache }
    }),
  addBlogToCache: (blog: Blog) =>
    set((state) => {
      const newCache = state.cache
  ...
  doFollow: async (usernameOrBlog: string) => {
    ...
    await firebase
      .firestore()
      .collection('users')
      .doc(state.user.data.username)
      .set(
        {
          following: state.user.data.following
            ? [...state.user.data.following, usernameOrBlog]
            : [usernameOrBlog],
        },
        { merge: true },
      )
  ...
  doUnfollow: async (usernameOrBlog: string) => {
    ...
    await firebase
      ...
      .set(
        {
          following: state.user.data.following.filter(
            (blog) => blog !== usernameOrBlog,
          ),
        },
        { merge: true },
      )
