...
function NewPost() {
  ...
  const [postToBlog, setPostToBlog] = useState(
    user && user.blogs && user.blogs.length === 1 ? user.blogs[0] : undefined,
  )
    ...
    ;(async () => {
      try {
        ...
        if (userRef.exists && userData) {
          if (
            userData.draft &&
            userData.draft.title &&
            userData.draft.content &&
            userData.draft.postTo &&
            userData.draft.slug
          ) {
      ...
      <Layout>
        <Container>
          ...
          <input
            ...
            {...customInputProps}
          />
          <InputInvalidMessage isValid={!submitted || title.length !== 0}>
          ...
          <EditorContent
            className={
              submitted && editor && editor.isEmpty ? styles.invalidEditor : ''
            }
            editor={editor}
