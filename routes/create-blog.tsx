...
function CreateBlog({ submitHandler }: CreateBlogProps) {
  ...
  const [backupBlogSlug, blogSlugTaken] = useBackupSlug(
    blogSlug,
    async (newSlug) => {
      ...
      return new Promise((resolve) => {
        typingDelayTimeout = setTimeout(async () => {
          try {
            const blogSlugDoc = await firebase
              .firestore()
              .collection('blogs')
              .doc(newSlug)
              .get()
            console.log(
              `checked if blog ${blogSlug} exists`,
              blogSlugDoc.exists,
            )
        ...
        <Container>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <input
              ...
              id="blogname"
              {...useCustomInputProps(!blogNameError)}
            />
            <InputInvalidMessage isValid={!blogNameRequiredError}>
              Please enter your blog name.
            </InputInvalidMessage>
            <InputInvalidMessage isValid={!blogNameLengthError}>
              Blog name must be 2-50 characters.
            </InputInvalidMessage>
            {blogSlug && (
              <p>
                Your blog will appear as {URL}/
                {blogSlugTaken ? backupBlogSlug : blogSlug}
              </p>
            )}
            {blogSlugTaken ? (
              <Box
                style={{
                  backgroundColor: '#f8a0b2',
                  border: '2px solid #cc0f35',
                }}
              >
                The URL for this blog name is taken. To get a matching blog URL,
            ...
            <textarea
              ...
              autoComplete="off"
              rows={3}
              {...useCustomInputProps(!descriptionLengthError)}
            />
            <InputInvalidMessage isValid={!descriptionLengthError}>
    ...
    ;(async () => {
      try {
        if (slug && user.auth && user.data) {
          const blogURLDoc = await firebase
            .firestore()
            .collection('blogs')
            .doc(slug)
            .get()
