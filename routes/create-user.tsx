...
function CreateUser({ submitHandler }: CreateUserProps) {
  ...
  const firstNameError =
    firstNameRequiredError || firstNameMaxLengthError || firstNamePatternError
  ...
  const lastNameError =
    lastNameRequiredError || lastNameMaxLengthError || lastNamePatternError
  ...
  const usernameError =
    ...
    usernameTakenError
  const usernameErrorWithoutValidate =
    usernameRequiredError || usernameLengthError || usernamePatternError
        ...
        <Container>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <input
              ...
              id="firstname"
              {...useCustomInputProps(!firstNameError)}
            />
            <InputInvalidMessage isValid={!firstNameRequiredError}>
              Please enter your first name.
            </InputInvalidMessage>
            <InputInvalidMessage isValid={!firstNameMaxLengthError}>
              Max length is 25.
            </InputInvalidMessage>
            <InputInvalidMessage isValid={!firstNamePatternError}>
            ...
            <input
              ...
              id="lastname"
              {...useCustomInputProps(!lastNameError)}
            />
            <InputInvalidMessage isValid={!lastNameRequiredError}>
              Please enter your last name.
            </InputInvalidMessage>
            <InputInvalidMessage isValid={!lastNameMaxLengthError}>
              Max length is 25.
            </InputInvalidMessage>
            <InputInvalidMessage isValid={!lastNamePatternError}>
              Please enter a valid name without special characters.
            </InputInvalidMessage>
            ...
            <input
                      ...
                      typingDelayTimeout = setTimeout(async () => {
                        try {
                          if (value && !usernameErrorWithoutValidate) {
                            console.log(
                              `checking if username ${value} is taken`,
                            )
              ...
              id="username"
              autoComplete="off"
              {...useCustomInputProps(!usernameError)}
            />
            <InputInvalidMessage isValid={!usernameRequiredError}>
              Please enter a username.
            </InputInvalidMessage>
            <InputInvalidMessage
              isValid={!usernameLengthError && !usernamePatternError}
            >
              Username must be 2-16 characters. Letters, numbers, hyphens, and
              underscores only.
            </InputInvalidMessage>
            <InputInvalidMessage isValid={!usernameTakenError}>
            ...
            <textarea
              ...
              autoComplete="off"
              rows={3}
              {...useCustomInputProps(!bioLengthError)}
            />
            <InputInvalidMessage isValid={!bioLengthError}>
